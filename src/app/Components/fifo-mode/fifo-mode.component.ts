...
export class FifoModeComponent implements OnInit {
  ...
  constructor(private fifoResquest: FifoRequestService) {
    this.filesForm = new FormGroup({
      fifoFiles: new FormArray([this.createFileFormGroup()]),
    });
  }
  ...
  createFileFormGroup(): FormGroup {
    return new FormGroup({
      ...
      vars: new FormArray([this.createVariableFormGroup()]),
    });
  }
  ...
  chargeMsgFifo(): void {
    ...
    const fifoFiles: List[] = this.fifoFiles.controls.map(
      (group: AbstractControl) => {
        const formGroup = group as FormGroup;
        const vars: Variable[] = (
          formGroup.get('vars') as FormArray
        ).controls.map((varGroup: AbstractControl) => {
    ...
    this.fifoResquest.putChargeFifo(fifoFiles).subscribe({
      next: (res) => {
        ...
        if (codeStatus !== 400) {
          this.msgChargeFifo =
            'Se ha cargado la lista de mensajes correctamente.';
        } else {
      ...
      error: (err) => {
        console.log(err);
        this.msgChargeFifo =
          'El sistema no está encendido / Modo List desactivado.';
      },
    });
