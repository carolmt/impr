...
import { PLC, PlC2, PLC3 } from '../../Interfaces/plcList.interface';
...
export class GeneralModeComponent implements OnInit {
  ...
  constructor(
    ...
    this.plcRegistersForm = new FormGroup({
      listPlcs: new FormArray([this.createRegisterFormGroup()]),
    });
  ...
  createRegisterFormGroup(): FormGroup {
    return new FormGroup({
      reg: new FormControl(''),
      values: new FormArray([this.createValuesFormGroup()]),
    });
  }
    ...
    this.requestService.getPrinterStatus().subscribe({
      next: (res) => {
        ...
        if (this.codeStatus == 200) {
          console.log(this.requestService.URL_BASE);
  ...
  chargeNewMsgNewVariables(): void {
    let codeStatus: number;
    const vars: Variable[] = this.vars.controls.map(
  ...
  writeValuesListInPlcRegisters(): void {
    ...
    const listPlcs: PLC3[] = this.listPlcs.controls.map(
      (group: AbstractControl) => {
        const formGroup = group as FormGroup;
        const values: number[] = (
          formGroup.get('values') as FormArray
        ).controls.map((valGroup: AbstractControl) => {
    ...
    this.requestService.putListOfValuesPlc(listPlcs).subscribe({
      next: (res) => {
        ...
        if (codeStatus !== 400) {
          this.mssgPlc =
            'Se han escrito los valores en los registros PLC correctamente.';
        } else {
          this.mssgPlc =
            'No se han podido escribir los valores en los registros PLC.';
        }
      },
      error: (err) => {
        console.log(err);
